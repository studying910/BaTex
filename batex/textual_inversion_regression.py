...
def score_computation(text_encoder, tokenizer, unet, vae, args, accelerator, weight_dtype, global_step,
    ...
    for i, prompt in enumerate(i2t_prompt_list):
        ...
        # get text features
        i2t_text_candidates = [prompt] * i2t_N  # .replace(args.placeholder_token, "")
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="Vector_dot",
        choices=["L2", "Cosine_similarity", "Vector_dot", "Random"],
...
def main():
    ...
    if args.second_token is None:
        if args.dist_type == "Vector_dot":
            ...
            candidate_embedding_matrix = token_embeds[vocab_ids]  # [args.mask_k, 768]
        elif args.dist_type == "Random":
            random_list = range(len(tokenizer) - 1)
            random_ids = random.sample(random_list, args.mask_k - 1)
            random_ids_tensor = torch.tensor(random_ids, dtype=torch.int64)
            random_matrix = token_embeds[random_ids_tensor]
            candidate_embedding_matrix = torch.cat((initialization_embedding.unsqueeze(0), random_matrix), 0)
    else:
