...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="Interpolation factor between learned embedding and nearest neighbour"
    )
    parser.add_argument(
        "--dist_type",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=20,
        help="Number of neighbours chose to update the parameters"
    )
    parser.add_argument(
        "--output_dir",
        type=str,
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(text_encoder):
                ...
                embeds = accelerator.unwrap_model(text_encoder).get_input_embeddings().weight.data[placeholder_token_id]
                if args.dist_type == "L2":
                    embeds_matrix = embeds.unsqueeze(0).expand(vocab_num, embeds.shape[0])  # embeds
                    residual_matrix = embeds_matrix - orig_embeds_params
                    vocab_dist = torch.norm(residual_matrix, 2, 1)
                    vocab_min, vocab_min_ids = torch.topk(vocab_dist, 1, 0, False)
                elif args.dist_type == "Vector_dot":
                    vocab_dist = torch.mm(orig_embeds_params, embeds.unsqueeze(1)).squeeze(1)
                    vocab_min, vocab_min_ids = torch.topk(vocab_dist, 1, 0, True)
                elif args.dist_type == "Cosine_similarity":
                    embeds_matrix = embeds.unsqueeze(0).expand(vocab_num, embeds.shape[0])
                    vocab_dist = torch.cosine_similarity(embeds_matrix, orig_embeds_params, 0)
                    vocab_min, vocab_min_ids = torch.topk(vocab_dist, 1, 0, True)

                pseudo_word = tokenizer.convert_ids_to_tokens(vocab_min_ids.item())
                neighbour_embedding = accelerator.unwrap_model(text_encoder).get_input_embeddings().weight.data[
                    vocab_min_ids.item()]

                interpolation_embedding = args.factor * embeds + (1 - args.factor) * neighbour_embedding
                with torch.no_grad():
                    accelerator.unwrap_model(text_encoder).get_input_embeddings().weight.data[
                        placeholder_token_id] = interpolation_embedding
