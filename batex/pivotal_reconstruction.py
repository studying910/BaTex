...
def parse_args():
    ...
    parser.add_argument(
        "--initializer_token_content", type=str, default=None, required=True,
        help="A token to use as initializer word for content embedding."
    )
    parser.add_argument(
        "--initializer_token_style", type=str, default=None, required=True,
        help="A token to use as initializer word for style embedding."
    )
...
def main():
    ...
    token_ids_content = tokenizer.encode(args.initializer_token_content, add_special_tokens=False)
    token_ids_style = tokenizer.encode(args.initializer_token_style, add_special_tokens=False)
    # Check if initializer_token is a single token or a sequence of tokens
    if len(token_ids_content) > 1:
        raise ValueError("The initializer token of content must be a single token.")
    if len(token_ids_style) > 1:
        raise ValueError("The initializer token of style must be a single token.")

    initializer_token_id_content = token_ids_content[0]
    initializer_token_id_style = token_ids_style[0]
    ...
    token_embeds[placeholder_token_id_content] = token_embeds[initializer_token_id_content]
    token_embeds[placeholder_token_id_style] = token_embeds[initializer_token_id_style]
    ...
    if accelerator.is_main_process:
        accelerator.init_trackers("pivotal_reconstruction", config=vars(args))
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(text_encoder):
                ...
                index_no_updates[placeholder_token_id_style] = False
                with torch.no_grad():
